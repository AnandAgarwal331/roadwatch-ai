-- Adds the "awaiting verification" count to kpi_summary(): repairs a crew has
-- marked done that no admin has approved yet - the number behind the new
-- "Awaiting verification" tile and report-queue filter (see
-- repositories/repair.ts's complaintIdsAwaitingVerification, which finds the
-- same set of complaints for the filtered list; this counts them for the
-- headline figure).
--
-- create or replace, not a new function: same name, arguments and return
-- type as the original in 20260922020000_kpi_summary_rpc.sql, so callers
-- (analytics.ts's kpiRow()) need no change, and the grants already on the
-- function carry over.
